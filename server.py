...
def fill_table(soup: BeautifulSoup, table: Tag, persons: Dict[str, List[Dict[str, str]]]) -> None:
    """
    Заполнение таблицы table спортсменами из словаря persons
    :param soup: soup
    :param table: Таблица, которую нужно заполнить
    :param persons: Словарь спортсменов
    """
    for category, category_persons in persons.items():
        # Ряд, предназначенный для названий категорий
        ...
        # Заполнение таблицы спортсменами
        position: int = 1
        old_weight: str = ""
